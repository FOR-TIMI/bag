...
const MiniBag = () => {
    ...
    () => (
      <div className="order-summary">
        <p className={`order-summary-value ${!noOfItems && "emptybag"}`}>
          Order value: <span>${orderValue}</span>
        </p>
        {!!noOfItems && (
  ...
  const handleRemoveFromBag = useCallback((id: string, size: string) => {
    removeFromBag(id, size);
  }, []);

  const renderBagItems = useCallback(
    () => (
      <div className="bag-items">
        {bagData.map((product) => (
          <div className="bag-item" key={`${product.id}-${product.size}`}>
            ...
            <div className="product-details">
              <div className="product-name-container">
                ...
                <button
                  ...
                  onClick={() => handleRemoveFromBag(product.id, product.size)}
                >
                  <svg
                    width="16"
                    height="16"
    ...
    [bagData, handleRemoveFromBag]
  );

  // Render shopping details
  const ShoppingDetails = useMemo(
    () =>
      noOfItems === 0 ? (
        <>
          {EmptyText}
    ...
    [noOfItems, EmptyText, OrderSummary, renderBagItems, EmptyBagButton]
  );

  return <div className="popover__bag-container">{ShoppingDetails}</div>;
};

export default React.memo(MiniBag);
